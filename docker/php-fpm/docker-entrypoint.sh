...
# Install project for the first time
if [ ! -d /var/www/html/public ]; then

  if [ -n "$REPO_PULL_TOKEN" ]; then
      git clone "https://x-token-auth:$REPO_PULL_TOKEN@bitbucket.org/referral-factory/encryption.git" /var/www/html
      cd /var/www/html
      # install composer
      # migrate
      # seed
      ...
      echo "You have not provided the REPO_PULL_TOKEN. Please get this token from Referral Factory Support and set the ENV in your Dockerfile"
      exit 1;
    fi
fi
